package com.twx.module_dynamic.saloon.adapter
...
import com.twx.module_dynamic.bean.LikeBean
 ...
 */
class SaloonAdapter(
    private val mList: MutableList<TrendSaloonList>,
    private val mDiyList: MutableList<LikeBean>,
) :
    RecyclerView.Adapter<SaloonAdapter.ViewHolder>(), View.OnClickListener {
    ...
    private fun initPic(holder: ViewHolder, position: Int) {
        ...
        if (mList[position].identity_status == 1) {
            Glide.with(mContext).load(com.twx.module_base.R.mipmap.icon_identify_success)
                .into(holder.identity)
        } else {
            Glide.with(mContext).load(com.twx.module_base.R.mipmap.icon_identify_non)
                .into(holder.identity)
        }
