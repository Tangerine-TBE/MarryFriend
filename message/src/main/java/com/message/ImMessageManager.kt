...
object ImMessageManager {
    ...
    fun observeNewMessage(
        ...
        observer: Observer<List<Message<out EMMessageBody>>?>,
    ) {
    ...
    fun getAllConversations(): List<ConversationsBean> {
        ...
        conversations.keys.forEach {
            ...
            ConversationsBean(it).apply {
                ...
                this.conversationType = ConversationType.toMyType(conversation.type)
                    ?: throw IllegalStateException("还未实现该会话")
            }
        }
    ...
    fun getHistoryMessage(
        ...
        pageSize: Int,
        msgId: String? = null,
    ): List<Message<out EMMessageBody>> {
        ...
        try {
            ...
            if (msgId != null && msgId.isNotBlank()) {
                ...
                if (index == -1) {
                    ...
                    resultList.addAll(conversation.loadMoreMsgFromDB(ramMsg[index].msgId,
                        pageSize - resultList.size))
                }
            } else {
                resultList.addAll(ramMsg)
                if (ramMsg.size < pageSize) {
                    resultList.addAll(conversation.loadMoreMsgFromDB(ramMsg.lastOrNull()?.msgId,
                        pageSize - ramMsg.size))
                }
            }
