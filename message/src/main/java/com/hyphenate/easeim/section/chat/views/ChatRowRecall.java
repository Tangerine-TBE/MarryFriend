...
public class ChatRowRecall extends EaseChatRow {
    ...
    protected void onSetUpView() {
        ...
        if (message.direct() == EMMessage.Direct.SEND&&(TextUtils.isEmpty(recaller)||((!TextUtils.isEmpty(recaller))&&TextUtils.equals(recaller, from)))) {
            messageStr = String.format(context.getString(com.hyphenate.easeui.R.string.msg_recall_by_self));
        } else if( (!TextUtils.isEmpty(recaller))&&!TextUtils.equals(recaller, from)) {
            messageStr = String.format(context.getString(com.hyphenate.easeui.R.string.msg_recall_by_another), recaller, from);
        }else{
            messageStr = String.format(context.getString(com.hyphenate.easeui.R.string.msg_recall_by_user), from);
        }
