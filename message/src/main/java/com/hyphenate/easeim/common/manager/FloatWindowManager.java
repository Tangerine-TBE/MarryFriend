...
public class FloatWindowManager {
    ...
    public static boolean tryJumpToPermissionPage(Context context) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            ...
            if (RomUtils.ROM_MIUI.equals(romName)) {
                return applyMiuiPermission(context);
            } else if (RomUtils.ROM_EMUI.equals(romName)) {
                return applyHuaweiPermission(context);
            } else if (RomUtils.ROM_VIVO.equals(romName)) {
                return applyVivoPermission(context);
            } else if (RomUtils.ROM_OPPO.equals(romName)) {
                return applyOppoPermission(context);
            } else if (RomUtils.ROM_QIKU.equals(romName)) {
                return apply360Permission(context);
            } else if (RomUtils.ROM_SMARTISAN.equals(romName)) {
                return applySmartisanPermission(context);
            } else if (RomUtils.ROM_COOLPAD.equals(romName)) {
                return applyCoolpadPermission(context);
            } else if (RomUtils.ROM_ZTE.equals(romName)) {
                return applyZTEPermission(context);
            } else if (RomUtils.ROM_LENOVO.equals(romName)) {
                return applyLenovoPermission(context);
            } else if (RomUtils.ROM_LETV.equals(romName)) {
