...
import com.twx.marryfriend.view.listener.SeekBarViewOnChangeListener
import com.twx.marryfriend.R
import com.twx.marryfriend.view.event.Down
import com.twx.marryfriend.view.event.Event
import com.twx.marryfriend.view.event.Move
import com.twx.marryfriend.view.event.Up
...
class GuideSeekBar : View {
    ...
    private fun touch(percent: Float, eventType: Event) {
        seekBarTouchListener?.touch(if (percent < 0) 0f else if (percent > 1) 1f else percent,
            eventType)
    }
    ...
    constructor(context: Context, attrs: AttributeSet?) : super(context, attrs) {
        ...
        prospectProgressBarHeight =
            seekTypedArray.getDimensionPixelOffset(R.styleable.SeekBarView_prospectProgressBarHeight,
                10)
        prospectProgressBarColor =
            seekTypedArray.getColor(R.styleable.SeekBarView_prospectProgressBarColor,
                prospectProgressBarColor)
        prospectProgressBarOffset =
            seekTypedArray.getDimensionPixelOffset(R.styleable.SeekBarView_prospectProgressBarOffset,
                5)
        backgroundProgressBarHeight =
            seekTypedArray.getDimensionPixelOffset(R.styleable.SeekBarView_backgroundProgressBarHeight,
                10)
        backgroundProgressBarColor =
            seekTypedArray.getColor(R.styleable.SeekBarView_backgroundProgressBarColor,
                backgroundProgressBarColor)
        backgroundProgressBarOffset =
            seekTypedArray.getDimensionPixelOffset(R.styleable.SeekBarView_backgroundProgressBarOffset,
                0)
        thumbColor =
    ...
    private fun drawThumbDetail(canvas: Canvas, currentStart: Float, currentEnd: Float, p0: Float) {
        ...
        if (thumbBackgroundStartColor != 0 || thumbBackgroundEndColor != 0) {
            ...
            val linearGradient = LinearGradient((currentStart - p0),
                0f,
                (currentEnd + p0),
                0f,
                intArray.toIntArray(),
    ...
    private fun drawBorder(canvas: Canvas, currentStart: Float, currentEnd: Float, p0: Float) {
        ...
        if (thumbBorderWidth > 0) {
            ...
            if (thumbBorderStartColor != 0 || thumbBorderEndColor != 0) {
                ...
                val linearGradient = LinearGradient((currentStart - p0 - thumbBorderWidth),
                    0f,
